...
| **2. Atributos Logísticos — Característica/Periculosidade** (Isento/Perigoso/Inflamável) | ❌ **FALTA** | spec `atributos-logisticos-shelf-life` (Req: campo dedicado no produto + uso no endereçamento). |
| **3. Shelf Life Total do fabricante** (ex.: 365 dias; calcular vencimento = fabricação + N) | ❌ **FALTA** | spec `atributos-logisticos-shelf-life`. Hoje a validade entra direta na conferência, sem cálculo fabricação+N. |
| **3. Shelf Life Mínimo de RECEBIMENTO / RLM** (recusa na doca se < X% de vida útil, ex.: 270 dias / 75%) | ❌ **FALTA** (parcial) | Hoje existe `Produto.shelfLifeMinimo` genérico usado na conferência (spec `conferencia-validade-produto`), mas NÃO a regra dedicada de % de vida útil no recebimento. spec `atributos-logisticos-shelf-life`. |
| **3. Shelf Life de EXPEDIÇÃO por cliente** (ex.: 60 dias; regra no perfil do cliente; aplicada no picking/FEFO) | ❌ **FALTA** | spec `atributos-logisticos-shelf-life`. FEFO já existe; falta a regra por cliente + bloqueio no picking. |
| **3. Trava de Quarentena automática** (faltando X dias p/ vencer, bloqueia lote p/ expedição + alerta) | ❌ **FALTA** | spec `atributos-logisticos-shelf-life`. Bloqueio manual/quarentena de endereço já existe; falta a automação por dias-a-vencer. |
