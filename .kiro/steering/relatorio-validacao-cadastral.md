...
| **3. Shelf Life Total do fabricante** (ex.: 365 dias; calcular vencimento = fabricação + N) | 🟡 **Cálculo pronto + cadastro; captura na doca pendente** | `Produto.shelfLifeTotalDias` no cadastro (ProdutoModal); helper `validar-validade-produto` calcula vencimento por fabricação. FALTA só o campo "data de fabricação" na UI da conferência (o helper e o backend já suportam). |
