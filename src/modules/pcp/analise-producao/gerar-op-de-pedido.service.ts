...
export async function gerarOpDePedido(
  ...
  // gerarOpFromOrcamento agora cuida de TUDO (materiais + etapas), nos dois
  // cenários: Repetição (BOM do produto vinculado) ou Especificação (materiais
  // do resultadoCalculo). Ver spec orcamento-grafico-materiais-repeticao.
  if (pedido.orcamentoOrigemId) {
    ...
    if (r) {
      opsGeradas.push({
        ...
        materiaisGerados: r.materiaisGerados,
