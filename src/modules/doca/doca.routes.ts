...
export async function docaRoutes(app: FastifyInstance) {
  app.get('/', async (request) => {
    ...
    const [data, total] = await Promise.all([
      prisma.doca.findMany({ where, skip: (q.page - 1) * q.limit, take: q.limit, orderBy: { criadoEm: 'asc' }, include: { deposito: { select: { descricao: true } } } }),
      prisma.doca.count({ where }),
    ])
  ...
  app.delete('/:id', async (request, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(request.params)
    await prisma.doca.delete({ where: { id } })
