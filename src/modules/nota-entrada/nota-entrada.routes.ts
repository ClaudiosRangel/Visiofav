...
export async function notaEntradaRoutes(app: FastifyInstance) {
  ...
  app.get('/:id', async (request, reply) => {
    ...
    const item = await prisma.notaEntrada.findUnique({
      where: { id },
  ...
  app.patch('/:id/status', async (request) => {
    ...
    return prisma.notaEntrada.update({ where: { id }, data: { status } })
  })

  app.delete('/:id', async (request, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(request.params)
    await prisma.itemNotaEntrada.deleteMany({ where: { notaEntradaId: id } })
