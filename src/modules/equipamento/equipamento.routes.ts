...
export async function equipamentoRoutes(app: FastifyInstance) {
  ...
  app.get('/', async (request) => {
    ...
    const [data, total] = await Promise.all([
      db.equipamentoMovimentacao.findMany({ where, skip: (q.page - 1) * q.limit, take: q.limit, orderBy: { descricao: 'asc' } }),
      db.equipamentoMovimentacao.count({ where }),
    ])
  ...
  app.post('/', async (request, reply) => {
    ...
    return reply.status(201).send(await db.equipamentoMovimentacao.create({ data: empresaId ? { ...data, empresaId } : data }))
  })
  ...
  app.put('/:id', async (request, reply) => {
    ...
    if (empresaId) {
      const existente = await db.equipamentoMovimentacao.findFirst({ where: { id, empresaId } })
    ...
    return db.equipamentoMovimentacao.update({ where: { id }, data })
  })
  ...
  app.delete('/:id', async (request, reply) => {
    ...
    await db.equipamentoMovimentacao.delete({ where: { id } })
