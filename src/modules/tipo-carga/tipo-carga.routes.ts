...
export async function tipoCargaRoutes(app: FastifyInstance) {
  ...
  app.get('/', async (request) => {
    ...
    const [data, total] = await Promise.all([
      db.tipoCarga.findMany({ where, skip: (q.page - 1) * q.limit, take: q.limit, orderBy: { descricao: 'asc' } }),
      db.tipoCarga.count({ where }),
    ])
  ...
  app.post('/', async (request, reply) => {
    ...
    return reply.status(201).send(await db.tipoCarga.create({ data: empresaId ? { ...data, empresaId } : data }))
  })
  ...
  app.put('/:id', async (request, reply) => {
    ...
    if (empresaId) {
      const existente = await db.tipoCarga.findFirst({ where: { id, empresaId } })
    ...
    return db.tipoCarga.update({ where: { id }, data })
  })
  ...
  app.delete('/:id', async (request, reply) => {
    ...
    await db.tipoCarga.delete({ where: { id } })
