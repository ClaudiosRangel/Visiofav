...
export async function tipoCarroceriaRoutes(app: FastifyInstance) {
  ...
  app.get('/', async (request) => {
    ...
    const [data, total] = await Promise.all([
      db.tipoCarroceria.findMany({ where, skip: (q.page - 1) * q.limit, take: q.limit, orderBy: { descricao: 'asc' } }),
      db.tipoCarroceria.count({ where }),
    ])
  ...
  app.post('/', async (request, reply) => {
    ...
    return reply.status(201).send(await db.tipoCarroceria.create({ data: empresaId ? { ...data, empresaId } : data }))
  })
  ...
  app.put('/:id', async (request, reply) => {
    ...
    if (empresaId) {
      const existente = await db.tipoCarroceria.findFirst({ where: { id, empresaId } })
    ...
    return db.tipoCarroceria.update({ where: { id }, data })
  })
  ...
  app.delete('/:id', async (request, reply) => {
    ...
    await db.tipoCarroceria.delete({ where: { id } })
