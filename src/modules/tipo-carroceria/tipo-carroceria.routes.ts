...
export async function tipoCarroceriaRoutes(app: FastifyInstance) {
  app.get('/', async (request) => {
    ...
    const [data, total] = await Promise.all([
      prisma.tipoCarroceria.findMany({ where, skip: (q.page - 1) * q.limit, take: q.limit, orderBy: { descricao: 'asc' } }),
      prisma.tipoCarroceria.count({ where }),
    ])
  ...
  app.post('/', async (request, reply) => {
    ...
    return reply.status(201).send(await prisma.tipoCarroceria.create({ data }))
  })

  app.put('/:id', async (request) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(request.params)
    const data = z.object({ descricao: z.string().optional(), status: z.boolean().optional() }).parse(request.body)
    return prisma.tipoCarroceria.update({ where: { id }, data })
  })

  app.delete('/:id', async (request, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(request.params)
    await prisma.tipoCarroceria.delete({ where: { id } })
