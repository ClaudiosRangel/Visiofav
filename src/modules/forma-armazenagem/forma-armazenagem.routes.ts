...
export async function formaArmazenagemRoutes(app: FastifyInstance) {
  ...
  app.get('/', async (request) => {
    ...
    const [data, total] = await Promise.all([
      db.formaArmazenagem.findMany({ where, skip: (q.page - 1) * q.limit, take: q.limit, orderBy: { descricao: 'asc' } }),
      db.formaArmazenagem.count({ where }),
    ])
  ...
  app.post('/', async (request, reply) => {
    ...
    return reply.status(201).send(await db.formaArmazenagem.create({ data: empresaId ? { ...data, empresaId } : data }))
  })
  ...
  app.put('/:id', async (request, reply) => {
    ...
    if (empresaId) {
      const existente = await db.formaArmazenagem.findFirst({ where: { id, empresaId } })
    ...
    return db.formaArmazenagem.update({ where: { id }, data })
  })
  ...
  app.delete('/:id', async (request, reply) => {
    ...
    await db.formaArmazenagem.delete({ where: { id } })
