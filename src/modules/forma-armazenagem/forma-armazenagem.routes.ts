...
export async function formaArmazenagemRoutes(app: FastifyInstance) {
  app.get('/', async (request) => {
    ...
    const [data, total] = await Promise.all([
      prisma.formaArmazenagem.findMany({ where, skip: (q.page - 1) * q.limit, take: q.limit, orderBy: { descricao: 'asc' } }),
      prisma.formaArmazenagem.count({ where }),
    ])
  ...
  app.post('/', async (request, reply) => {
    ...
    return reply.status(201).send(await prisma.formaArmazenagem.create({ data }))
  })

  app.put('/:id', async (request) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(request.params)
    const data = z.object({ descricao: z.string().optional(), status: z.boolean().optional() }).parse(request.body)
    return prisma.formaArmazenagem.update({ where: { id }, data })
  })

  app.delete('/:id', async (request, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(request.params)
    await prisma.formaArmazenagem.delete({ where: { id } })
