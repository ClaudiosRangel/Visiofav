...
export async function classificacaoProdutoRoutes(app: FastifyInstance) {
  ...
  app.post('/', async (request, reply) => {
    ...
    return reply.status(201).send(await prisma.classificacaoProduto.create({ data }))
  })

  app.put('/:id', async (request) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(request.params)
    const data = z.object({ descricao: z.string().optional(), status: z.boolean().optional() }).parse(request.body)
    return prisma.classificacaoProduto.update({ where: { id }, data })
  })

  app.delete('/:id', async (request, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(request.params)
    await prisma.classificacaoProduto.delete({ where: { id } })
