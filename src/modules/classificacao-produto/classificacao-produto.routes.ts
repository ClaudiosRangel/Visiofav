...
export async function classificacaoProdutoRoutes(app: FastifyInstance) {
  ...
  app.get('/', async (request) => {
    ...
    const [data, total] = await Promise.all([
      db.classificacaoProduto.findMany({ where, skip: (q.page - 1) * q.limit, take: q.limit, orderBy: { descricao: 'asc' } }),
      db.classificacaoProduto.count({ where }),
    ])
  ...
  app.post('/', async (request, reply) => {
    ...
    return reply.status(201).send(await db.classificacaoProduto.create({ data: empresaId ? { ...data, empresaId } : data }))
  })
  ...
  app.put('/:id', async (request, reply) => {
    ...
    if (empresaId) {
      const existente = await db.classificacaoProduto.findFirst({ where: { id, empresaId } })
    ...
    return db.classificacaoProduto.update({ where: { id }, data })
  })
  ...
  app.delete('/:id', async (request, reply) => {
    ...
    await db.classificacaoProduto.delete({ where: { id } })
