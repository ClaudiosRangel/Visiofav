...
<NFe xmlns="http://www.portalfiscal.inf.br/nfe">
  ...
  // GET /:id/danfe — Visualizar NF-e em formato DANFE (HTML)
...
<html><head><meta charset="UTF-8"><title>DANFE - NF-e ${nfe.numero}</title>
<style>
  body { font-family: Arial, sans-serif; font-size: 11px; margin: 20px; }
  .danfe { border: 2px solid #000; padding: 10px; max-width: 800px; margin: 0 auto; }
  .header { display: flex; justify-content: space-between; border-bottom: 2px solid #000; padding-bottom: 10px; margin-bottom: 10px; }
  .header-left { flex: 1; }
  .header-center { flex: 1; text-align: center; border-left: 1px solid #000; border-right: 1px solid #000; padding: 0 10px; }
  .header-right { flex: 1; text-align: right; padding-left: 10px; }
  .empresa-nome { font-size: 16px; font-weight: bold; }
  .danfe-title { font-size: 20px; font-weight: bold; }
  .section { border: 1px solid #000; margin-bottom: 5px; padding: 5px; }
  .section-title { font-weight: bold; font-size: 9px; color: #666; text-transform: uppercase; }
  .section-value { font-size: 12px; font-weight: bold; }
  .row { display: flex; gap: 5px; }
  .col { flex: 1; border: 1px solid #ccc; padding: 3px; }
  ...
  .chave { font-family: monospace; font-size: 10px; letter-spacing: 1px; }
  @media print { body { margin: 0; } .danfe { border: none; } }
</style></head><body>
<div class="danfe">
  <div class="header">
    <div class="header-left">
      <div class="empresa-nome">${empresa.razaoSocial}</div>
      <div>${empresa.logradouro || ''}, ${empresa.numero || ''} - ${empresa.bairro || ''}</div>
      <div>${empresa.cidade || ''} - ${empresa.uf || ''} CEP: ${empresa.cep || ''}</div>
      <div>CNPJ: ${empresa.cnpj} IE: ${empresa.inscEstadual || ''}</div>
    </div>
    <div class="header-center">
      <div class="danfe-title">DANFE</div>
      <div>Documento Auxiliar da<br>Nota Fiscal Eletrônica</div>
      <div style="margin-top:5px">0 - ENTRADA<br><b>1 - SAÍDA</b></div>
      <div style="margin-top:5px;font-size:14px;font-weight:bold">Nº ${String(nfe.numero).padStart(9, '0')}</div>
      <div>Série ${nfe.serie}</div>
    </div>
    <div class="header-right">
      <div class="section-title">CHAVE DE ACESSO</div>
      <div class="chave">${nfe.chaveAcesso || '— XML não gerado —'}</div>
      <div style="margin-top:10px">
        <div class="section-title">PROTOCOLO DE AUTORIZAÇÃO</div>
        <div>${nfe.protocolo || 'Sem protocolo (homologação)'}</div>
      </div>
    </div>
  </div>
  ...
  <div class="section">
    ...
    <div class="row">
      <div class="col" style="flex:3"><div class="section-title">Nome/Razão Social</div><div class="section-value">${cliente?.razaoSocial || '—'}</div></div>
      <div class="col"><div class="section-title">CNPJ/CPF</div><div class="section-value">${cliente?.cpfCnpj || '—'}</div></div>
    </div>
    <div class="row">
      <div class="col" style="flex:2"><div class="section-title">Endereço</div><div>${cliente?.logradouro || ''}, ${cliente?.numero || ''}</div></div>
      <div class="col"><div class="section-title">Bairro</div><div>${cliente?.bairro || ''}</div></div>
      <div class="col"><div class="section-title">Cidade</div><div>${cliente?.cidade || ''}</div></div>
      <div class="col" style="flex:0.3"><div class="section-title">UF</div><div>${cliente?.uf || ''}</div></div>
    </div>
  </div>
  ...
  <table>
    <thead>
      <tr>
        <th>Código</th><th>Descrição</th><th>NCM</th><th>CFOP</th><th>UN</th><th>Qtd</th><th>Vl. Unit.</th><th>Vl. Total</th>
      </tr>
    </thead>
    <tbody>
      ${nfe.itens.map((item) => `<tr>
        <td>${item.cProd}</td><td>${item.xProd}</td><td>${item.ncm}</td><td>${item.cfop}</td>
        <td>${item.uCom}</td><td>${Number(item.qCom).toFixed(2)}</td><td>${Number(item.vUnCom).toFixed(4)}</td><td>${Number(item.vProd).toFixed(2)}</td>
      </tr>`).join('')}
    </tbody>
  </table>

  <div class="total">VALOR TOTAL DA NOTA: R$ ${valorTotal.toFixed(2)}</div>

  <div class="section" style="margin-top:10px">
    <div class="section-title">INFORMAÇÕES COMPLEMENTARES</div>
    <div>NF-e gerada pelo VisioFab ERP - Ambiente de ${nfe.ambiente === 1 ? 'Produção' : 'Homologação'}</div>
    <div>Status: ${nfe.status} | Emissão: ${new Date(nfe.criadoEm).toLocaleDateString('pt-BR')}</div>
  </div>
</div>
</body></html>`
